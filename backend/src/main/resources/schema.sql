...
CREATE TABLE IF NOT EXISTS pathology_no_rule (
  id BIGINT PRIMARY KEY AUTO_INCREMENT, name VARCHAR(100) NOT NULL, business_type VARCHAR(50) NOT NULL DEFAULT 'PATHOLOGY_CASE',
  prefix VARCHAR(30), year_format VARCHAR(30) NOT NULL DEFAULT 'yyyy', `separator` VARCHAR(10) NOT NULL DEFAULT '-',
  sequence_digits INT NOT NULL DEFAULT 6, start_sequence INT NOT NULL DEFAULT 1, enabled TINYINT NOT NULL DEFAULT 1,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS report_plan (
  ...
  cron_expression VARCHAR(100), execution_time TIME, spec_id BIGINT, priority INT NOT NULL DEFAULT 5, max_retry INT NOT NULL DEFAULT 4,
