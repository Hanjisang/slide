...
系统以完整闭环和最少基础设施为优先。业务 API 位于单个 Spring Boot 服务；Python Worker 保持统一切片门面，Go Parser 隔离多格式算法与可选厂家依赖。所有服务通过 Compose 内部网络连接。
...
Browser -> /api/slides/{id}/tiles/{level}/{x}/{y}
        -> Spring Boot 统一代理 -> slide-worker
                                  +-> SVS -> OpenSlide
                                  +-> KFB/TMAP/MDSX/DMETRIX/FENLAN/ZYP/SDPC/CSP -> Go Parser
```

Worker 将 MinIO 对象缓存到 `slide_cache:/data/slides/{slideId}`。Go Parser 以只读方式挂载同一 volume，不重复下载对象；HWP/TRON 在缺 SDK 时保留不可操作能力，不影响 SVS 或纯 Go 格式。

## v0.3 Parser 服务

Go Parser 使用 `net/http` 提供 health、formats、analyze、thumbnail、label、macro 和 tile API。Registry 只把有构造器的格式交给解析器，格式扩展名只是路由入口，解析器仍需校验 header/magic 和元数据。
...
- Go Parser 只解析固定缓存根下的数字 ID 目录，拒绝符号链接、非普通文件和路径逃逸。
- 随机读取、元数据、层数、尺寸、索引数、编码图片和 HTTP 输出均有显式上限；Parser panic 被转换为失败响应。
